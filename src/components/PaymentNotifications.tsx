...
function pickBuyer(): { name: string; countryLabel: string; countryEmoji: string } {
  const isUSA = Math.random() < 0.8; // majority USA
  if (isUSA) {
    const name = `${getRandom(usaFirstNames)} ${getRandom(usaLastNames)}`;
    return { name, countryLabel: 'USA', countryEmoji: '🇺🇸' };
  }
  const other = getRandom(otherCountries);
  const flags: Record<string, string> = { GB: '🇬🇧', CA: '🇨🇦', AU: '🇦🇺', DE: '🇩🇪', FR: '🇫🇷' };
  return { name: `${getRandom(usaFirstNames)} ${getRandom(usaLastNames)}`, countryLabel: other.name, countryEmoji: flags[other.cc] };
}
...
const PaymentNotifications: FC = () => {
  ...
  const [payload, setPayload] = useState<{ buyer: string; country: string; countryEmoji: string; provider: Provider; product: string; price: number } | null>(null);
